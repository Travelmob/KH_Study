...
/*
    ...
    => 해당 테이블에 모든 컬럼에 값을 추가하고자 할 때 내가 직접 값을 제시해서 한 행을 INSERT할 수 있음
...
VALUES(900, '홍길동', '550101-1010101', 'hong@naver.com', '01011223344', 'D1', 'J1', 'S3', 40000000, 0.2, 200, SYSDATE, NULL, DEFAULT);
...
/*
    ...
    한 행 단위로 추가되기 때문에 선택을 하지않은 컬럼은 기본적으로 NULL값이 들어감
    (단, DEFAULT(기본값)이 지정되어있을 경우 기본값이 들어감)
    주의할점 : NOT NULL제약조건이 걸려있는 컬럼은 반드시 선택해서 직접 값을 제시해야함
            -> NOT NULL제약조건이 걸려있는 컬럼도 기본값이 지정되어있다면 선택 안할 수 있음
*/
...
INSERT ALL
  INTO EMP_JOB VALUES(EMP_ID, EMP_NAME, JOB_NAME)  -- 9개행이 추가
  INTO EMP_DEPT VALUES(EMP_ID, EMP_NAME, JOB_NAME) -- 9개행이 추가
SELECT EMP_ID, EMP_NAME, JOB_NAME, DEPT_TITLE
  FROM EMPLOYEE
  JOIN JOB USING(JOB_CODE)
  JOIN DEPARTMENT ON (DEPT_CODE = DEPT_ID)
...
/*
    ...
    UPDATE 테이블명
       SET 컬럼명 = 바꿀값
          ,컬럼명 = 바꿀값
          ,... => 여러 개의 컬럼값 동시 변경 가능(,로 나열해야함 AND 는 안됨!!!!)
     WHERE 조건; => WHERE절은 생략 가능, 생략 시 전체 모든 행의 데이터가 다 바뀜
*/
...
-- 전체 행 DEPT_TITLE컬럼의 모든 행을 다 전략기획팀으로 바꿔라
...
-- EMP_SARALY 테이블에 홍길동 사원의 급여를 1000만원으로 변경
UPDATE EMP_SALARY
   SET SALARY = 10000000
 WHERE EMP_NAME = '홍길동';
...
UPDATE EMPLOYEE
   SET EMP_ID = 200
 WHERE EMP_NAME = '송종기'; -- PRIMARY KEY 제약조건 위배!!
...
/*
    ...
    [ 표현법 ]
    DELETE FROM 테이블명
     WHERE 조건; => WHERE 절은 생략 가능, 생략 시 해당 테이블 전체 행 삭제
*/

DELETE FROM EMPLOYEE; -- EMPLOYEE테이블에 모든 행 삭제
...
DELETE FROM EMP_SALARY; -- 0.038초

ROLLBACK;

TRUNCATE TABLE EMP_SALARY; -- 0.039초
