/*
    ...
    - 수정할 내용
    1) 컬럼 추가 / 수정/ 삭제
    2) 제약조건 추가 / 삭제 => 수정은 불가(수정하고 싶으면 삭제 한다음에 다시 추가)
    3) 테이블명 / 컬럼명 / 제약조건명
*/

-- 1) 컬럼 추가 / 수정 / 삭제
-- 1_1) 컬럼 추가 (ADD) : ADD 추가할컬럼명 데이터타입 DEFAULT 기본값(DEFAULT 기본값은 생략 가능)
...
-- 예 ) 문자 -> 숫자(X) // 문자열 사이즈 축소(X) // 문자열 사이즈 확대(OOOOO)


-- DEPT_TITLE컬럼의 데이터타입을 VARCHAR2(40)로
-- LOCATION_ID 컬럼의 데이터타입을 VARCHAR2(5)로
-- LANME컬럼의 기본값을 '미국'
...
ALTER TABLE DEPT_COPY2 DROP COLUMN LNAME; -- 마지막 컬럼 삭제는 오류가 발생!! : 테이블에 최소 한개의 컬럼은 존재해야함!!!
---------------------------------------------------------------------------------
...
--                          기존테이블명을 생략 가능!! ALTER TABLE 기존테이블명에서 기술하기 때문에!!
...
-- 단, 어딘가에서 참조되괴 있는 부모테이블들은 삭제되지 않는다.
