/*
    ...
    (자주 사용하는 길이가 긴 SELECT문을 저장해두면 매 번 다시 기술할 필요없이 CIEW를 이용해서 편하게 사용가능!!)
...
-- VW_EMPLOYEE 뷰에 BONUS컬럼이 존재하지 않기 때문에 오류 발생
...
/*
    ...
    만약에 중복된 이름의 뷰가 있다면 해당 뷰를 변경(갱신)하는 옵션
*/
...
/*
    * 뷰컬럼에 별칭 부여
    서브쿼리의 SELECT절에 함수나 산술연산식이 기술되어있는경우 반드시 별칭 지정
*/
...
/*
    * 생성한 뷰를 이용해서 DML(DELETE, INSERT, UPDATE) 사용가능
    단, 뷰를 통해서 DML구문을 수행하게 되면 실제 데이터가 담겨지는 테이블(베이스테이블)에도 적용이 된다!!   
*/
...
/*
    ...
    2) 뷰에 정의되지 않은 컬럼 중에 베이스테이블에 NOT NULL제약조건이 지정되어있는 경우
    3) 산술연산식 또는 함수를 통해서 정의한 경우
    4) GROUP BY나 그룹함수 절이 포함된 경우
...
/*
    * VIEW 옵션
    ...
    2) FORCE / NOFORCE
       ...
       - NOFORCE(생략시 기본값) : 서브쿼리에 기술된 테이블이 반드시 존재하만 뷰가 생성
    3) WITH CHECT OPTION : 서브쿼리에 조건절에 기술된 내용이 만족하는 값으로만 DML이 가능
                           조건에 부합하지 않은 값으로 수정하는 경우 오류 발생
    4) WITH READ ONLY : 뷰에 대해 조회만 가능(DML 수행 불가)
*/

-- 2) FORCE / NOFORCE
CREATE OR REPLACE FORCE VIEW VM_TEST
...
SELECT * FROM VW_TEST;
-- 오류 발생
-- 단, 접속 항목에서 KH - 뷰를 열어보면 VM_TEST뷰가 생성이 된 것은 확인이 가능!!
...
SELECT * FROM VW_TEST;
-- T테이블을 생성 후 다시 뷰를 조회하면 정상적으로 확인이 가능!!
