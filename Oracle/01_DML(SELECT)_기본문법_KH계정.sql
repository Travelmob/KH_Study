...
-- 소문자로 해도 무방, 단, 대문자로 쓰는 습관을 들이도록하자...........
...
-- EMPLOYEE 테이블로부터 직원명, 월급, 보너스, 보너스가 포함된 연봉((보너스 * 월급 + 월급) * 12)
...
-- EMPLOYEE테이블로부터 사번, 사원명, 급여 단위 조회하기
...
/*
    ...
    (단, SELECT절에 DISTINCT구문에 단 한개만 사용 가능하다.)
*/
...
-- EMPLOYEE테이블로부터 부서코드가 D9가 아닌 사원들의 사원명, 부서코드, 급여 조회
SELECT EMP_NAME, DEPT_CODE, SALARY
  FROM EMPLOYEE
 WHERE DEPT_CODE != 'D9';
 --WHERE DEPT_CODE <> 'D9';
...
--> SELECT절에 부여한 별칭을 WHERE절에서 사용할 수 없음
-------------------------------------------------------
/*
    < 논리연산자 >
...
------------------- 실습문제 -------------------
-- 1. 이름이 '연'으로 끝나는 사원들의 이름, 입사일을 조회
...
/*
    ...
    [ 표현법 ]
    비교대상컬럼 IS NULL : 컬럼값이 NULL 인 경우
    비교대상컬럼 IS NOT NULL : 컬럼값이 NULL이 아닌 경우
*/

-- 보너스를 받지 않는 사원들(BONUS컬럼값이 NULL)의 사번, 이름, 급여, 보너스
...
-- 사수도 없고 부서배치도 받지않은 사원들의 모든 컬럼 조회
SELECT *
  FROM EMPLOYEE
...
/*
    ...
    SELECT 조회할 컬럼, 컬럼, ...
     ...
     -- NULLS FIRST : 정렬하고자하는 컬럼값에 NULL이 있을경우 해당 NULL값들을 앞으로 배치(내림차순 정렬일 경우 기본값)
     -- NULLS LAST :: 정렬하고자하는 컬럼값에 NULL이 있을경우 해당 NULL값을들 뒤로 배치(오름차순 정렬일 경우 기본값)
...
SELECT *
 ...
 --ORDER BY BONUS ASC; -- ASC는 기본적으로 NULLS LAST임을 알 수 있다.
 --ORDER BY BONUS ASC NULLS FIRST;
 --ORDER BY BONUS DESC; -- DESC는 기본적으로 NULLS FIRST임을 알 수 있다.
 ORDER BY BONUS DESC, SALARY ASC;
-- 첫번째로 제시한 정렬기준의 컬럼값이 일치할 경우 두번째 정렬기준을 가지고 다시 정렬할 수 있음
