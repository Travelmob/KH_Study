...
/*
    ...
    * 캐시메모리 : 미리 발생될 값들을 생성해두는 저장공간
                매 번 호출할 때마다 새롭게 번호를 생성하는것보다
                캐시 메모리 공간에 미리 생성된 값들을 가져다 쓰게되면 속도가 빠름!
                단, 접속이 끊기고 나서 재접속 후 기존에 생성되있던 값들은 날아가고 없음!!!
*/
/*
    * 접두사
...
/*
    ...
    시컨스명.ENXTVAL : 시퀀스값을 증가시키고 증가된 시퀀스의 값
                    => 시퀀스 생성 후 첫 NEXTVALDMS STARTWITH로 지정된 시작값이 만들어짐
                    => 기존의 시퀀스에서 INCREMENT BY 값만큼 증가된 값
                    (시퀀스명.CURRVAL + INCREMENT BY 값)
*/

SELECT SEQ_EMPNO.CURRVAL FROM DUAL;
-- NEXTVAL을 한번이라도 수행하지 않는 이상  CURRVAL을 수행할 수 없음
-- CURRVAL은 마지막에 성공적으로 수행한 NEXTVAL의 값으 저장해서 보여주는 임시값이기 때문!
...
SELECT * FROM USER_SEQUENCES;
-- LAST__NUMBER : 현재 상황에서 NEXTVAL을 수행할 경우 예정값
SELECT SEQ_EMPNO.NEXTVAL FROM DUAL;

SELECT SEQ_EMPNO.NEXTVAL FROM DUAL;
-- 지정한 MAXVALUE값(310)을 초과했기 때문에 오류 발생

SELECT SEQ_EMPNO.CURRVAL FROM DUAL;
-- 마지막으로 성공적으로 생성한 NEXTVAL값

------------------------------------------------------------------
/*
    3. 시퀀스 수정
    
    CREATE -> ALTER절로 INCREMENT BY, MAXVALUE, MINVALUES, CYCLE, CASHE 수정가능
